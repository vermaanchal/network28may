...
//     setEditValues({ serviceCharges: "", repairCharges: "", remarks: "" });
//   };
...
const ReviewBatchPage = () => {
  ...
  useEffect(() => {
    ...
    // Prefill form fields with selectedBatchData
    setInvoiceNo(selectedBatchData.invoiceNo || "");
    setInvoiceDate(selectedBatchData.invoiceDate || "");
    setInvoiceAmount(selectedBatchData.invoiceAmount ? selectedBatchData.invoiceAmount.toString() : "");
    setCaseCount(selectedBatchData.caseCount ? selectedBatchData.caseCount.toString() : "");
    ...
    if (extension === "csv") {
      Papa.parse(file, {
        ...
        complete: (results) => {
          ...
          setInvoiceDate(row["Invoice Date"] || selectedBatchData.invoiceDate || "");
          setInvoiceAmount(row["Invoice Amount"] ? row["Invoice Amount"].toString() : selectedBatchData.invoiceAmount ? selectedBatchData.invoiceAmount.toString() : "");
          setCaseCount(row["Case Count"] ? row["Case Count"].toString() : selectedBatchData.caseCount ? selectedBatchData.caseCount.toString() : "");
        },
        error: (err) => {
          console.error("CSV parsing error:", err);
          toast.error("Failed to read CSV file.");
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Convert invoiceAmount and caseCount to strings for validation
